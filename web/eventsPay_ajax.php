...
$result['error']=0;
  ...
  $hidregid=trim($_POST['hidregid']);
	$cardDt=$_POST['cardDt'];
       ...
       if($tblrow!="" && $cardDt!="" && $hidregid !="" && $hidregid>0)
	{   $Ndt=date('Y-m-d H:i:s');
      ...
      if($chkCardDetalToken[0]['tCnts']>0) /** check valid token **/
      {
           ...
           $GetEventsPackage=$db->query("SELECT count(id) As valC,name,events_name,price,id,	is_active FROM events WHERE 	is_active=:active AND id=:id AND NOT name=:name AND price > :price",array("active"=>"1","id"=>"$tblrow","name"=>"","price"=>"0"));

        if($GetEventsPackage[0]['valC']==1)
         {
            ...
            $chkCouponValid=$db->query("SELECT count(id) as Cnt,id,code,dis_type_id,discount,coupon_from,coupon_to,active FROM coupon  WHERE    curdate()  BETWEEN coupon_from AND coupon_to  AND code='".$couponVal."' AND discount >0 AND active=1");
             if($chkCouponValid[0]['Cnt']>0) 
             {
              ...
              if($ChkvalidforUser[0]['UCnt']==0)
              {
                ...
                $orgPrice=$GetEventsPackage[0]['price'];
                 ...
                 elseif($chkCouponValid[0]['dis_type_id']==2)
                 {
             ...
             {
              $price=($GetEventsPackage[0]['price']*100);
              $orgPrice=$GetEventsPackage[0]['price'];
              $disFinalPrice=$GetEventsPackage[0]['price'];
             }


              $stripe_eventsplan_id=$GetEventsPackage[0]['name'];
		          $inVoiceId=$sesUID.time();
		          $events_id=$GetEventsPackage[0]['id'];

		$description=json_encode(array("CustomInvoice"=>"$inVoiceId","UserId"=>"$sesUID","plan"=>"$stripe_eventsplan_id","events_id"=>"$tblrow","price"=>"$orgPrice","CustomerId"=>"$customer_stripe_id","discoutAmt"=>"$discountCalculatPrice","regFormId"=>"$hidregid")) ;
		...
		$keyVals=$chkkey[0]['s_key'];
			...
			if($paidResponse==1 && $PaymentResult=="succeeded" && $res_id !="" )
                        { $curDt=date("Y-m-d H:i:s");
                          $insertPack=$db->query("INSERT INTO user_pay_pack_subscribe 		 (user_id,events_id,token_id,describ,invoice_id,res_result,res_paid,res_id,price,pak_Intervel_id,created_at,plan_name,plan_price,dis_type_id,discount,discount_code,discount_applied,discount_reduction_amt,e_reg_id) 
VALUES(:user_id,:events_id,:token_id,:describ,:invoice_id,:res_result,:res_paid,:res_id,:price,:pak_Intervel_id,:created_at,:plan_name,:plan_price,:dis_type_id,:discount,:discount_code,:discount_applied,:discount_reduction_amt,:e_reg_id) ",
			array("user_id"=>"$sesUID",
			"events_id"=>"$tblrow","token_id"=>"$tk_id",
			"describ"=>"$description","invoice_id"=>"$inVoiceId",
			"res_result"=>"$PaymentResult","res_paid"=>"$paidResponse",
			"res_id"=>"$res_id","price"=>"$disFinalPrice","pak_Intervel_id"=>"3","created_at"=>"$curDt","plan_name"=>"$stripe_eventsplan_id","plan_price"=>"$orgPrice","dis_type_id"=>"$dis_type_id","discount"=>"$discount_actual","discount_code"=>"$discount_code","discount_applied"=>"$discount_applied","discount_reduction_amt"=>"$discountCalculatPrice","e_reg_id"=>"$hidregid"));
			  echo "3#$#Events registered successfully";
                         exit();
			}
                        else
                        {
...
$chkCardDetalToken=$db->query("SELECT count(tk_id) AS tCnts,token_key,customer_stripe_id,tk_id FROM user_card_token WHERE user_id=:user_id AND tk_id=:tk_id AND NOT token_key=:token_key AND NOT customer_stripe_id=:customer_stripe_id ",array("user_id"=>"$sesUID","tk_id"=>"$cardDt","token_key"=>"","customer_stripe_id"=>""));
	 ...
	 exit;
         ...
         if($pak_Intervel_id==2) /** One time Plan  **/
	{
                 $customer_stripe_id=$chkCardDetalToken[0]['customer_stripe_id'];
                 $tk_id=$chkCardDetalToken[0]['tk_id'];
                 $token_key=$chkCardDetalToken[0]['token_key'];
