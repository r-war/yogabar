<div class="navbar-default {$navhome|default:'navbar navbar-static-top'}">  
  {*<div class="container-fluid">  *}
    <div class="navbar-header">
      <button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse"> <span class="icon-bar"></span> <span class="icon-bar"></span> <span class="icon-bar"></span> </button>
    </div>
    <a class="navbar-brand" href="#" title="Yogabar" alt="Yogabar">Yogabar</a>
    <div class="navbar-collapse collapse ">
      <ul class="nav navbar-nav pull-right">
        <li class="{$home}"><a href="index.php">Home</a></li>
        <li class="{$package}"><a href="package.php">Package</a></li>
        <li class="{$blog}"><a href="blog.php">Blog</a></li>
        <li class="{$teacher}"><a href="teacher.php">Teacher</a></li>
        <li class="dropdown {$gallery}">
          <a href="gallery.php" class="dropdown-toggle" data-toggle="dropdown">Gallery<b class="caret"></b></a>
          <ul class="dropdown-menu">
            <li><a href="gallery.php">List gallery</a></li>
            <li><a href="gallery_cat.php">Gallery Category</a></li>
          </ul>
        </li> 
        <li class="{$banner}"><a href="banner_add.php">Home Page Popup </a></li>  
        <li class="{$coupon}"><a href="coupon.php">Coupon</a></li>        
        <li class="{$events}"><a href="events.php">Events</a></li>  
        
        <li class="{$faq}"><a href="faq_add.php">FAQ</a></li>  
        <li class="{$change_pwd}"><a href="change_pwd.php">Change password</a></li>

<li class="dropdown {$Report}"> 
        <a href="profile.php" class="dropdown-toggle" data-toggle="dropdown">Report <b class="caret"></b></a>
        <ul class="dropdown-menu">
         <li class="{$users}"><a href="reg_users.php">Users</a></li> 
           <li class="{$payreP}"><a href="report.php">Payment Report</a></li> 
 <li class="{$evereP}"><a href="report_events.php">Events Report</a></li>
          
        </ul>
      </li>

        <li class=""><a href="logout.php">Logout</a></li>        
      </ul>
    </div>
  {*</div>*}
</div>
