      ...
      <div class="white-bg">
        <form action="package_checkout.php" id='packFrm' name="packFrm" method="post">
          ...
          <ul>
            ...
            {if {$package['group_type_id']}==1 && ({$package['id']}!=70 && {$package['id']}!=82 && {$package['id']}!=81
            && {$package['id']}!=85 )}

            <li>

              <input type="radio" {$package['subcrib']} name="amount" id='opt_{$package[' id']}' data-toggle="modal"
                data-target="#myModalNorm" onclick="radioEvent('{$package['name']}', this.value,'{$package['id']}','{$ChkLogin}');"
                value="{$package['price']}">
          ...
          <ul>
            {foreach $getAllSubunSubList as $key => $package}
            {if {$package['group_type_id']}==1 && ({$package['id']}==70 )}
          ...
          <ul>
            {foreach $getAllSubunSubList as $key => $package}
            {if {$package['group_type_id']}==1 && ({$package['id']}==82 || {$package['id']}==81 || {$package['id']}==85
            )}
