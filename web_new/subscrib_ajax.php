...
$chkCardDetalToken=$db->query("SELECT count(tk_id) AS tCnts,token_key,customer_stripe_id,tk_id FROM user_card_token WHERE user_id=:user_id AND tk_id=:tk_id AND NOT token_key=:token_key AND NOT customer_stripe_id=:customer_stripe_id ",array("user_id"=>"$sesUID","tk_id"=>"$cardDt","token_key"=>"","customer_stripe_id"=>""));
	...
	{ 
 	...
 	$planPaymentProcess=0;
		...
		$description=json_encode(array("CustomInvoice"=>"$inVoiceId","UserId"=>"$sesUID","plan"=>"$stripe_plan_id","package_id"=>"$pack_id","price"=>"$price","CustomerId"=>"$customer_stripe_id","tblTkId"=>"$tk_id")) ;
 		$PaymentResult="";
		$paidResponse="";
		...
		\Stripe\Stripe::setApiKey($keyVals);  /** Create a Payment start **/
			...
			if($paidResponse==1 && $PaymentResult=="succeeded" && $res_id !="" )
                        {
                          $insertPack=$db->query("INSERT INTO user_pay_pack_subscribe 		 (user_id,plan_id,token_id,describ,invoice_id,res_result,res_paid,res_id,price,pak_Intervel_id,pak_total_month) 
VALUES(:user_id,:plan_id,:token_id,:describ,:invoice_id,:res_result,:res_paid,:res_id,:price,:pak_Intervel_id,:pak_total_month) ",
			...
			"res_id"=>"$res_id","price"=>"$price","pak_Intervel_id"=>"$pak_Intervel_id","pak_total_month"=>"$pak_total_month"));
