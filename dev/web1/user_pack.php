...
// select  (CASE WHEN a.res_subcri_result='active' AND a.res_subcri_id !="" THEN 1 ELSE 0 end) as subcriSucc,(CASE WHEN a.res_paid=1 AND res_result='succeeded' AND a.res_id!="" THEN 1 ELSE 0 END ) as Paysucc,b.name,b.price, a.pay_id,token_id,plan_id,res_paid,res_id,res_result,res_subcri_result,res_subcri_id,a.un_subscrib,a.created_at FROM user_pay_pack_subscribe a,packages b WHERE a.user_id=1 AND a.plan_id=b.id
 ...
 $getAllSubunSubList=$db->query("SELECT  (CASE WHEN a.res_subcri_result=:res_subcri_result AND NOT a.res_subcri_id =:res_subcri_id THEN :S1 ELSE :S2 END) as subcriSucc,a.pak_Intervel_id,(CASE WHEN a.res_paid=:res_paid AND a.res_result=:res_result AND NOT  a.res_id=:res_id THEN :P1 ELSE :P2 END ) as Paysucc, plan_name as name,plan_price as price, a.pay_id,token_id,plan_id,res_paid,res_id,res_result,res_subcri_result,res_subcri_id,a.un_subscrib,a.created_at FROM user_pay_pack_subscribe a  WHERE a.user_id=:user_id AND a.res_paid=:res_paid1 AND a.res_result=:res_result1 AND NOT  a.res_id=:res_id1 ",array("res_subcri_result"=>"active","res_subcri_id"=>"","S1"=>"1","S2"=>"0","res_paid"=>"1","res_result"=>"succeeded","res_id"=>"","P1"=>"1","P2"=>"0","user_id"=>"$userId","res_paid1"=>"1","res_result1"=>"succeeded","res_id1"=>"")); 


 /*echo "SELECT  (CASE WHEN a.res_subcri_result='active' AND NOT a.res_subcri_id ='' THEN 1 ELSE 0 END) as subcriSucc,a.pak_Intervel_id,(CASE WHEN a.res_paid=1 AND a.res_result='succeeded' AND NOT  a.res_id='' THEN 1 ELSE 0 END ) as Paysucc,'' as name,a.price, a.pay_id,token_id,plan_id,res_paid,res_id,res_result,res_subcri_result,res_subcri_id,a.un_subscrib,a.created_at FROM user_pay_pack_subscribe a  WHERE a.user_id=$userId AND a.res_paid=1 AND a.res_result='succeeded' AND NOT  a.res_id='' ";  */
//$varVal='"';
/*$getAllSubunSubList=$db->query("SELECT  (CASE WHEN a.res_subcri_result='active' AND NOT a.res_subcri_id ='' THEN 1 ELSE 0 END) as subcriSucc,a.pak_Intervel_id,(CASE WHEN a.res_paid=1 AND a.res_result='succeeded' AND NOT  a.res_id='' THEN 1 ELSE 0 END ) as Paysucc,'' as name,a.price, a.pay_id,token_id,plan_id,res_paid,res_id,res_result,res_subcri_result,res_subcri_id,a.un_subscrib,a.created_at,substring_index(substr(describ,locate('plan',describ)+char_length('plan')+3),'$varVal',1) as plan FROM user_pay_pack_subscribe a  WHERE a.user_id=$userId AND a.res_paid=1 AND a.res_result='succeeded' AND NOT  a.res_id='' "); */
