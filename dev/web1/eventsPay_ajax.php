...
$result['error']=0;
	...
	{   $Ndt=date('Y-m-d H:i:s');
      ...
      if($chkCardDetalToken[0]['tCnts']>0) /** check valid token **/
      {
           ...
           $GetEventsPackage=$db->query("SELECT count(id) As valC,events_name,price,id,	is_active FROM events WHERE 	is_active=:active AND id=:id AND NOT events_name=:events_name AND price > :price",array("active"=>"1","id"=>"$tblrow","events_name"=>"","price"=>"0"));

        if($GetEventsPackage[0]['valC']==1)
         {


               $stripe_eventsplan_id=$GetEventsPackage[0]['events_name'];
		...
		$description=json_encode(array("CustomInvoice"=>"$inVoiceId","UserId"=>"$sesUID","plan"=>"$stripe_eventsplan_id","events_id"=>"$tblrow","price"=>"$orgPrice","CustomerId"=>"$customer_stripe_id")) ;
		$PaymentResult="";
		$paidResponse="";
		...
		$keyVals=$chkkey[0]['s_key'];
			...
			if($paidResponse==1 && $PaymentResult=="succeeded" && $res_id !="" )
                        { $curDt=date("Y-m-d H:i:s");
                          $insertPack=$db->query("INSERT INTO user_pay_pack_subscribe 		 (user_id,events_id,token_id,describ,invoice_id,res_result,res_paid,res_id,price,pak_Intervel_id,created_at,plan_name,plan_price) 
VALUES(:user_id,:events_id,:token_id,:describ,:invoice_id,:res_result,:res_paid,:res_id,:price,:pak_Intervel_id,:created_at,:plan_name,:plan_price) ",
			array("user_id"=>"$sesUID",
			"events_id"=>"$events_id","token_id"=>"$tk_id",
			"describ"=>"$description","invoice_id"=>"$inVoiceId",
			"res_result"=>"$PaymentResult","res_paid"=>"$paidResponse",
			"res_id"=>"$res_id","price"=>"$orgPrice","pak_Intervel_id"=>"3","created_at"=>"$curDt","plan_name"=>"$stripe_eventsplan_id","plan_price"=>"$orgPrice"));
